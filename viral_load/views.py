    ...
    def create_lab_order_results(self,results):
        if results:
            ...
            for result in results:
                result = result.rstrip('\n').split('\t')
                try:
                    update_viral_load = ViralLoad.objects.get(accession_number=result[0])
                    update_viral_load.released_date = result[1] 
                    update_viral_load.results =  result[2]   
                    update_viral_load.save()
                except ViralLoad.DoesNotExist:
